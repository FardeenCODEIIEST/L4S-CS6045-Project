...
\noindent\textbf{Repository link:} \url{https://github.com/FardeenCODEIIEST/L4S-CS6045-Project}
...
BMv2 provides the target features needed for the prototype. The ingress pipeline assigns packets to one of two priority queues through BMv2 priority metadata when \texttt{simple\_switch} is launched with two queues. The egress pipeline reads queue metadata, including \texttt{deq\_qdepth}, \texttt{enq\_qdepth}, and \texttt{deq\_timedelta}. The P4 program can also read and write registers that are visible to the control plane through the BMv2 CLI. These mechanisms define the feasible design: class separation is performed in ingress, marking and telemetry export are performed in egress, and threshold adaptation is performed by a controller outside the dataplane.
